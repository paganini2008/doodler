...
import io.doodler.quartz.scheduler.JobOperationsException;

import static io.doodler.quartz.JobConstants.DEFAULT_JOB_SERVICE_NAME;
...
public class RestJobOperations implements JobOperations {
    ...
    public Date addJob(JobDefination jobDefination) throws Exception {
        ResponseEntity<ApiResult<Date>> responseEntity = restTemplate.exchange(
                ...
                HttpMethod.POST, new HttpEntity<Object>(jobDefination, getDefaultHeader()),
    ...
    public Date modifyTrigger(String triggerName, String triggerGroup,
                              Date startTime, long period, int repeatCount, Date endTime) throws Exception {
        ...
        ResponseEntity<ApiResult<Date>> responseEntity = restTemplate.exchange(
                ...
                HttpMethod.POST, new HttpEntity<Object>(triggerDefination),
                new ParameterizedTypeReference<ApiResult<Date>>() {
                });
        if (responseEntity.getStatusCode().isError()) {
            throw new JobOperationsException("Job Server Error: " + responseEntity.toString());
    ...
    public void runNow(String jobName, String jobGroup, String parameter) throws Exception {
        ...
        jobRun.setJobGroupName(jobGroup);
        jobRun.setParameter(parameter);
        ResponseEntity<ApiResult<String>> responseEntity = restTemplate.exchange(
                URI.create(String.format("http://%s/job/man/run", DEFAULT_JOB_SERVICE_NAME)),
                HttpMethod.POST, new HttpEntity<Object>(jobRun), new ParameterizedTypeReference<ApiResult<String>>() {
                });
        if (responseEntity.getStatusCode().isError()) {
            throw new JobOperationsException("Job Server Error: " + responseEntity.toString());
        }
