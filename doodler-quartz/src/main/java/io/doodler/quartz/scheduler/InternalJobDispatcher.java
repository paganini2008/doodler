...
public class InternalJobDispatcher implements JobDispatcher {
    ...
    @Override
    public void directCall(String guid, JobSignature jobSignature) {
        ...
        if (responseEntity != null && responseEntity.getStatusCode() == HttpStatus.OK) {
            if (log.isInfoEnabled()) {
                log.info("Job Executor Response: {}", responseEntity.getBody().toString());
            }
        }
    }

    @Override
    public void dispatch(String guid, JobSignature jobSignature) {
    ...
    private HttpHeaders getHeaders(JobSignature jobSignature) {
        ...
        if (ArrayUtils.isNotEmpty(jobSignature.getHeaders())) {
            int index;
            for (String line : jobSignature.getHeaders()) {
                index = line.indexOf("=");
                if (index > 0) {
