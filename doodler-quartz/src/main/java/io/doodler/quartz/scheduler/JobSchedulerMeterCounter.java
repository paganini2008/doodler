	...
	public void onSchedulerStateChange(SchedulerStateChangeEvent event) {
		...
		switch (event.getSchedulerStateEventType()) {
			case JOB_ADDED:
				...
				mmap.get("quartz.job.added.count", jobGroup).increment();
				break;
			case JOB_DELETED:
				jobGroup = ((JobKey) parameter).getGroup();
				mmap.get("quartz.job.deleted.count", jobGroup).increment();
				break;
			case JOB_SCHEDULED:
				jobGroup = ((Trigger) parameter).getJobKey().getGroup();
				mmap.get("quartz.job.scheduled.count", jobGroup).increment();
				break;
			case SCHEDULER_ERROR:
				...
				mmap.get("quartz.scheduler.error.count", jobGroup).increment();
				break;
			default:
				break;
		}
