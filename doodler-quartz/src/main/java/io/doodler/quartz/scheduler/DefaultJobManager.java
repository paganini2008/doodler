...
import io.doodler.id.IdGenerator;
...
public class DefaultJobManager implements JobManager, DisposableBean {
    ...
    public synchronized Date addJob(JobDefination jobDefination) throws SchedulerException {
        if (isJobExists(jobDefination.getJobName(), jobDefination.getJobGroup())) {
            ...
            throw new JobOperationsException("Job '" + jobKey + "' is existed");
        }
        ...
        JobSignature jobSignature = new JobSignature(id, jobDefination.getJobGroup(), jobDefination.getJobName(),
                jobDefination.getClassName(), jobDefination.getMethod(), jobDefination.getHeaders(), applicationName,
        ...
        if (trigger == null) {
            trigger = SimpleScheduleBuilder.simpleSchedule().withMisfireHandlingInstructionNextWithRemainingCount()
                    ...
                    .withRepeatCount(triggerDefination.getRepeatCount() >= 0 ? triggerDefination.getRepeatCount()
    ...
    @Override
    public synchronized Date modifyJob(JobDefination jobDefination) throws Exception {
        if (isJobExists(jobDefination.getJobName(), jobDefination.getJobGroup())) {
            TriggerDefination triggerDefination = jobDefination.getTriggerDefination();
            return modifyTrigger(triggerDefination.getTriggerName(), triggerDefination.getTriggerGroup(),
                    triggerDefination.getStartTime(),
                    triggerDefination.getPeriod(), triggerDefination.getRepeatCount(), triggerDefination.getEndTime());
        }
        return null;
    }

    @Override
    public synchronized Date modifyCronJob(JobDefination jobDefination) throws Exception {
        if (isJobExists(jobDefination.getJobName(), jobDefination.getJobGroup())) {
            TriggerDefination triggerDefination = jobDefination.getTriggerDefination();
            return modifyTrigger(triggerDefination.getTriggerName(), triggerDefination.getTriggerGroup(),
                    triggerDefination.getCron(),
                    triggerDefination.getStartTime(), triggerDefination.getEndTime());
        }
        return null;
    }

    @Override
    public int countOfJobs(String jobGroup, String jobNamePattern) throws Exception {
    ...
    @Override
    public int countOfTriggers(String triggerGroup, String triggerNamePattern) throws Exception {
    ...
    private JobDefination convert2JobDefination(JobDetail jobDetail) {
        ...
        jobDefination.setHeaders(jobSignature.getHeaders());
    ...
    @Override
	public PageVo<JobStatusVo> pageForJobStatus(String jobGroup, String jobNamePattern, int pageNumber, int pageSize) throws Exception {
