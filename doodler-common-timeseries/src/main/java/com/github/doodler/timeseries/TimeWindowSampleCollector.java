 ...
 * @Version 1.0.0
 */
public class TimeWindowSampleCollector<C, D, E extends Metric, T extends Sampler<E>> implements SampleCollector<E, T> {

    private final TimeWindowMap<T> samplers;
    private final Supplier<T> supplier;

    public TimeWindowSampleCollector(int span,
                                     TimeWindowUnit timeWindowUnit,
                                     TimeZone timeZone,
                                     int maxSize,
                                     Supplier<T> supplier,
                                     OverflowDataStore<C, D, E, T> overflowDataStore) {
        this.samplers = new TimeWindowMap<>(span, timeWindowUnit, timeZone, maxSize, overflowDataStore);
    ...
    public Map<String, T> samplers(TimeZone timeZone, DateTimeFormatter dtf) {
        return samplers.entrySet().stream().sorted(Map.Entry.comparingByKey()).collect(LinkedHashMap::new,
                (m, e) -> m.put(
                        e.getKey().atZone(timeZone != null ? timeZone.toZoneId() :
                                DEFAULT_TIMEZONE.toZoneId()).toLocalDateTime().format(dtf != null ? dtf :
