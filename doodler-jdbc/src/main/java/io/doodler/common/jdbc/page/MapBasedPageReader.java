package io.doodler.common.jdbc.page;
...
import io.doodler.common.jdbc.impexp.PooledConnectionFactory;
...
public class MapBasedPageReader implements PageReader<Map<String, Object>> {
    ...
    @Override
    public List<Map<String, Object>> list(int pageNumber, int maxResults) throws SQLException {
        Connection connection = null;
        try {
            connection = connectionFactory.getConnection();
            int offset = (pageNumber - 1) * maxResults;
            String boundSql = String.format("%s limit %d offset %d", sql, maxResults, offset);
            return JdbcUtils.fetchAll(connection, boundSql, args);
        } finally {
