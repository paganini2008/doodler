...
public class WebAppAuthenticationService implements AuthenticationService {
    ...
    public String signIn(AbstractAuthenticationToken authenticationToken, AuthenticationPostHandler postHandler,
        ...
        try {
            ...
            if (cleanToken(user, request, response)) {
                authenticationAspects.forEach(a -> a.onLogout(user, LogoutReason.ABNORMAL_LOGOUT, request, response));
            }
            ...
            AuthenticationInfo authInfo = new AuthenticationInfo(user.getId(), user.getUsername(), user.getPlatform(),
            ...
            authenticationAspects.forEach(a -> a.onFailure(authenticationToken, e, request, response));
            throw e;
        }
    }
    ...
    public void signOut(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            InternalAuthenticationToken authenticationToken = (InternalAuthenticationToken) SecurityUtils.getAuthentication();
            ...
            try {
                ...
                authenticationAspects.forEach(a -> a.onLogout(userDetails, LogoutReason.NORMAL_LOGOUT, request, response));
            } catch (Exception e) {
    ...
    public void handleOffline(RedisKeyExpiredEvent<?> e) {
        ...
        try {
            ...
            authenticationAspects.forEach(a -> a.onLogout(userDetails, LogoutReason.SESSION_EXPIRED, null, null));
        } catch (Exception ee) {
            if (log.isErrorEnabled()) {
