...
public class HttpSecurityConfig {
    ...
    static final String[] DEFAULT_PERMITTED_URLS = new String[]{
            "/",
            "/error",
            "/ping",
            "/login",
    ...
    private final HttpSecurityCustomizer httpCustomizer;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, 
        ...
        http.csrf().disable()
                ...
                .permitAll()
                .antMatchers(getPermittedUrls())
                .permitAll()
                .mvcMatchers(getPermittedUrls())
                .servletPath(servletContextPath)
                .permitAll()
                .requestMatchers(new WhiteListRequestMatcher(whiteListProperties, restClientProperties))
                .permitAll()
...
//                .hasAuthority(ROLE_SUPPORTER)
                ...
                .and()
                .exceptionHandling()
                .authenticationEntryPoint(new ForbiddenAuthenticationEntryPoint(objectMapper, messageLocalization))
        ...
        http.addFilterBefore(new WebCorsFilter(), ChannelProcessingFilter.class)
                .addFilter(new MixedAuthenticationFilter(authenticationManager, 
                		securityClientProperties,
                        Collections.emptyList(), 
                        tokenStrategy,
                        redisOperations,
                        handlerExceptionResolver))
                .addFilterAt(platformRememberMeAuthenticationFilter(authenticationManager, rememberMeServices),
    ...
    public PlatformRememberMeAuthenticationFilter platformRememberMeAuthenticationFilter(
            AuthenticationManager authenticationManager,
            AbstractRememberMeServices rememberMeServices) {
        return new PlatformRememberMeAuthenticationFilter(authenticationManager, rememberMeServices);
    }
}