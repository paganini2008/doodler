...
import static io.doodler.common.Constants.DEFAULT_MAXIMUM_RESPONSE_TIME;
import static io.doodler.common.Constants.REQUEST_HEADER_REQUEST_ID;
...
public class WebRequestStdout extends WebRequestCompletionAdvice {
    ...
    private final WebRequestLoggingProperties webRequestLoggingProperties;
    ...
    private void doLog(HttpServletRequest request, HttpServletResponse response, Object handler, Exception e) {
    ...
    private StringBuilder logRequestAndResponse(HttpServletRequest request, HttpServletResponse response, Object handler,
        ...
        if (handler instanceof HandlerMethod) {
            ...
            handlerDescription = String.format("[%s] %s#%s(%s args)", requestId, handlerMethod.getBeanType().getSimpleName(),
        ...
        for (Map.Entry<String, List<String>> entry : httpRequestInfo.getRequestHeaders().entrySet()) {
        ...
        log(str, "[%s] <--- END HTTP %s (%s ms) ", handlerDescription, HttpStatus.valueOf(response.getStatus()), elapsed);
        if (e != null) {
            log(str, "[%s] <--- ERROR %s: %s", handlerDescription, e.getClass().getSimpleName(), e.getMessage());
    ...
    private Level getLoggingLevel(HttpServletRequest request, HttpServletResponse response, Exception e, long elapsed) {
    ...
    private boolean matchesLoggingUrls(HttpServletRequest request) {
        if (CollectionUtils.isEmpty(webRequestLoggingProperties.getPaths())) {
            return false;
        }
        String requestPath = request.getRequestURI().replaceFirst(servletContextPath, "");
        if (webRequestLoggingProperties.getPaths().contains(requestPath)) {
            return true;
        }
        for (String pathPattern : webRequestLoggingProperties.getPaths()) {
