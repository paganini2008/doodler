...
public class JdbcJobLogService extends JobLogService implements InitializingBean {
    ...
    private static final String SQL_SELECT_COUNT_LOG =
            "select count(1) from qrtz_job_run_log where guid=:guid";
    private static final String SQL_UPDATE_LOG =
            "update qrtz_job_run_log set job_executor=:jobExecutor, executor_instance=:executorInstance, status=:status, end_time=:endTime, retry=:retry, errors=:errors where guid=:guid";
    private static final String SQL_INSERT_LOG =
            "insert into qrtz_job_run_log (id,job_group,job_name,trigger_name,trigger_group,description,class_name,url,method,headers,initial_parameter,guid,job_scheduler,scheduler_instance,start_time) values (:id,:jobGroup,:jobName,:triggerName,:triggerGroup,:description,:className,:url,:method,:headers,:initialParameter,:guid,:jobScheduler,:schedulerInstance,:startTime)";
    private static final String SQL_PAGE_COUNT_LOG =
            "select count(1) from (select * from qrtz_job_run_log where 1=1 %s) as T";
    private static final String SQL_PAGE_LOG =
            "select * from qrtz_job_run_log where 1=1 %s order by start_time desc limit %d offset %d";
    private static final String SQL_SUMMARY_LOG =
            "select status, count(*) as count from qrtz_job_run_log where retry=false group by status";
    ...
    public PageVo<JobLog> readLog(JobLogQuery logQuery) throws SQLException {
        ...
        PageResponse<JobLog> pageResponse =
                pageReader.list(PageRequest.of(logQuery.getPageNumber(), logQuery.getPageSize()));
    ...
    private class JobLogPageReader implements PageReader<JobLog> {
        ...
        @Override
        public PageContent<JobLog> list(int pageNumber, int offset, int limit, Object nextToken)
                throws SQLException {
            StringBuilder whereClause = new StringBuilder();
            if (StringUtils.isNotBlank(logQuery.getJobName())) {
                whereClause.append(" and job_name=:jobName");
            ...
            List<JobLog> jobLogs =
                    jdbcTemplate.query(selectSql, new BeanPropertySqlParameterSource(logQuery),
