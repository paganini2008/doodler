package io.doodler.common.swagger;

import com.github.xiaoymin.knife4j.spring.annotations.EnableKnife4j;
import com.google.common.base.Predicate;
import com.google.common.base.Predicates;

import io.doodler.common.swagger.SwaggerResourceProperties.Authorization;
...
public class SwaggerResourceConfig {
    ...
    private static final String[] DEFAULT_EXCLUDED_PATH = new String[]{"/error", "/ping", "/favicon.ico", "/actuator/**",
            "/security/**", "/test/**", "/job/**"};
    ...
    public Docket doc(SwaggerResourceProperties swaggerProperties) {
        ...
        if (CollectionUtils.isNotEmpty(swaggerProperties.getPath())) {
            basePath = swaggerProperties.getPath().stream().map(path -> PathSelectors.ant(path)).collect(
                    Collectors.toList());
        } else {
        ...
        if (CollectionUtils.isNotEmpty(swaggerProperties.getExcludedPath())) {
            excludedPath = swaggerProperties.getExcludedPath().stream().map(path -> PathSelectors.ant(path)).collect(
                    Collectors.toList());
        } else {
            excludedPath = Arrays.stream(DEFAULT_EXCLUDED_PATH).map(path -> PathSelectors.ant(path)).collect(
                    Collectors.toList());
        }
    ...
    public ApiInfo apiInfo(SwaggerResourceProperties swaggerProperties) {
        ApiInfoBuilder builder = new ApiInfoBuilder().title(swaggerProperties.getTitle()).description(
        ...
        if (swaggerProperties.getContact() != null) {
            io.doodler.common.swagger.SwaggerResourceProperties.Contact c = swaggerProperties.getContact();
    ...
    private List<Parameter> getHeaders() {
        ...
        ParameterBuilder tokenPair = new ParameterBuilder().name("Authorization").order(1).description(
                "Spring Security Authorization").modelRef(new ModelRef("string")).parameterType("header").required(false);
    ...
    private SecurityScheme oauthSecuritySchema(SwaggerResourceProperties swaggerProperties) {
        ...
        return new OAuthBuilder().name(swaggerProperties.getAuthorization().getName()).grantTypes(grantTypes).scopes(
    ...
    SecurityContext securityContext(SwaggerResourceProperties swaggerProperties) {
        ...
        if (CollectionUtils.isNotEmpty(authorization.getExcludedPath())) {
            excludedPath = authorization.getExcludedPath().stream().map(path -> PathSelectors.ant(path)).collect(
                    Collectors.toList());
        }
    ...
    SecurityReference defaultAuth(SwaggerResourceProperties swaggerProperties) {
        ...
        return SecurityReference.builder().reference(swaggerProperties.getAuthorization().getName()).scopes(
                authorizationScopeList).build();
    }
