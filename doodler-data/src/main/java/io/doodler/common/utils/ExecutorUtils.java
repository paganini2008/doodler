...
public class ExecutorUtils {
	...
	public <T> List<T> submitAll(Executor executor, Collection<Callable<T>> callables, boolean failfast) {
	...
	public <T> List<T> submitAll(Executor executor, Collection<Callable<T>> callables, long timeout, TimeUnit timeUnit,
			boolean failfast) {
		final List<T> results = new ArrayList<>();
		final CountDownLatch latch = new CountDownLatch(callables.size());
		for (final Callable<T> callable : callables) {
	...
	public <T> void submitAll(ExecutorService executor, Collection<Callable<T>> callables, Consumer<T> consumer)
			throws Exception {
		if (consumer != null) {
	...
	public <T> void submitAll(ExecutorService executor, Collection<Callable<T>> callables, long timeout, TimeUnit timeUnit,
