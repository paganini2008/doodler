...
public class LruMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Serializable {
	...
	public LruMap(final Map<K, V> delegate, final int maxSize, final RemovalListener<V> removalListener) {
		this.delegate = delegate;
		this.keys = new ConcurrentLinkedHashMap.Builder<K, Object>().maximumWeightedCapacity(maxSize).weigher(Weighers.singleton())
				.listener((eldestKey, v) -> {
